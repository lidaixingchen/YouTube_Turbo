...
2. **缺乏原生级交互反馈与 Tooltip**：
   ...
   - 缺少开关型功能（如「单曲循环」、「深色主题」）的即时激活高亮态指示。
3. **强制同步布局与重排开销**：
...
- **原生悬浮 Tooltip 状态提示区**：
  - 在工具箱面板顶部中央动态呈现 YouTube 原生文字气泡（如 `截取画面`、`单曲循环 (已开启)`、`画中画`），无系统延迟，平滑淡入淡出。
...
- **显式三态状态机（`CLOSED` | `HOVER_OPEN` | `PINNED_OPEN`）**：
  ...
  - **声明式动作响应契约**：
    ...
    - 状态开关型动作（`dismissOnExecute: false`，如单曲循环、主题切换）：执行后保持面板展开并即时刷新激活状态。
...
3. **交互与状态机测试**：
   ...
   - 验证执行「单曲循环」与「主题切换」后面板保持展开且状态高亮实时切换；
